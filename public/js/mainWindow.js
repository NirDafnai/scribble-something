        ...
        inputbox.addEventListener("keyup", (event) => 
        {
            if (event.key == "Enter") 
            {
                ...
                if (inputbox.value != "" && !this.timeEnded && !this.isDrawer) 
                {
                    ...
                    console.log(this.username + ": " + inputbox.value)
                    a.innerText = this.username + ": " + inputbox.value;
        ...
        ipcRenderer.on("sendWinner", (event) => 
        {
            ...
            document.querySelectorAll("#player").forEach((player) => 
            {
                if ((parseInt(player.querySelector(".score").textContent) > highestScore[1])) 
                {
                    ...
                    console.log("winner stuff " +  parseInt(player.querySelector(".score").textContent));
                }
            }
            );
            let hasHighScore = 0;
