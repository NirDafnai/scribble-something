...
class Canvas
{
	...
	addListeners()
	{
		...
		ipcRenderer.on("addUsers", (event, users, midGame) => 
		{
			...
			users.forEach((user, index) => 
			{
				...
				if (midGame) 
				{
					let user_and_score = user.split("|")
					user = user_and_score[0];
					user_score = user_and_score[1];
				}
		...
		ipcRenderer.on("removeUser", (event, username) => 
		{
		...
		document.querySelectorAll(".wordbuttons").forEach((button) => 
		{
			button.addEventListener("mousedown", (event) => 
			{
				...
				innerCircle.style.top = "22px";
				innerCircle.style.left = "22px";
				this.paintBucketPressed = false;
				this.drawing = false;
				this.isDrawer = true;
				this.clearCanvas();
		...
		ipcRenderer.on("midGameSettings", (event, drawer, hiddenWord, rounds, counter) => 
		{
			...
			document.querySelectorAll("#player").forEach((player) => 
			{
				...
				if (player.querySelector("#playername").textContent == drawer) 
				{
					player.querySelector("#pencil").style.visibility = "visible";
				}
			}
			);
			if (hiddenWord == "none") 
			{
				document.getElementById("timer").innerText = "Waiting..."
				document.getElementById("word").innerText = drawer + " is choosing a word...";
			}
			else 
			{
				counter = counter - 2;
				...
				var event = new Event('timerStart');
				timerLabel.dispatchEvent(event);
				document.getElementById("word").innerText = hiddenWord;
				this.timerCounter = 90;
			}
			document.getElementById("rounds").textContent = "Round " + rounds + "/3";
		}
		);
		...
		{
			ipcRenderer.send("send_canvas_back", this.canvas.toDataURL());
		}
		);
